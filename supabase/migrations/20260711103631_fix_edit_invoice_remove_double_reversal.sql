-- =============================================================================
-- FIX: edit_invoice — remove double-reversal of AR/Revenue/COGS
--
-- Bug: edit_invoice Step 1b posts explicit reversal JEs (invoice_edit) for
-- AR+Revenue, and Step 1c posts reversal JEs for COGS. Then it also DELETEs
-- the original 'invoice' JEs with balance rollback. This double-reverses:
-- the deletion already reverses the original posting, so the explicit
-- reversal JEs are an extra reversal that causes AR/Revenue/COGS to be
-- understated after edits.
--
-- Fix: Remove Steps 1b and 1c (explicit reversal JEs). The deletion of
-- original 'invoice' JEs with balance rollback IS the reversal.
-- Keep Step 1a (stock restore) and Step 1d (refund payment records).
-- =============================================================================
...
BEGIN
  ...
  -- 1b. (REMOVED) No explicit reversal JEs for AR+Revenue.
  --     The original 'invoice' JEs are deleted below with balance rollback,
  --     which fully reverses them. Posting explicit reversals would double-reverse.

  -- 1c. (REMOVED) No explicit reversal JEs for COGS.
  --     Same reason as 1b.
  ...
  -- This IS the reversal for AR+Revenue and COGS.
  -- =========================================================================
  FOR v_je_id IN
